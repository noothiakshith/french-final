import React, { useState } from 'react';
import { submitRemedialExercise } from '../api/remedial';
import styles from './Exercise.module.css'; // Reuse existing Exercise styles

const RemedialExercise = ({ exercise, onCorrectAnswer, isAlreadyCorrect }) => {
  const [userAnswer, setUserAnswer] = useState('');
  const [result, setResult] = useState(isAlreadyCorrect ? 'correct' : null);
  const [isLoading, setIsLoading] = useState(false);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!userAnswer.trim() || result) return;

    setIsLoading(true);
    try {
      const response = await submitRemedialExercise(exercise.id, userAnswer);
      if (response.data.isCorrect) {
        setResult('correct');
        onCorrectAnswer(exercise.id, response.data.chapterCompleted);
      } else {
        setResult('incorrect');
      }
    } catch (error) {
      console.error("Failed to submit remedial exercise", error);
    } finally {
      setIsLoading(false);
    }
  };

  const containerClasses = `${styles.container} ${result === 'correct' ? styles.correct : ''} ${result === 'incorrect' ? styles.incorrect : ''}`;

  return (
    <div className={containerClasses}>
      <p className={styles.question}>{exercise.question}</p>
      <form onSubmit={handleSubmit} className={styles.form}>
        <input
          type="text"
          value={userAnswer}
          onChange={(e) => setUserAnswer(e.target.value)}
          placeholder="Your answer..."
          disabled={!!result}
          className={styles.input}
        />
        <button type="submit" disabled={!!result || isLoading} className={styles.button}>
          {isLoading ? 'Checking...' : 'Check'}
        </button>
      </form>

      {result === 'correct' && (
        <div className={`${styles.feedback} ${styles.feedbackCorrect}`}>
          <strong>Correct!</strong> Great job working through this remedial exercise.
        </div>
      )}
      {result === 'incorrect' && (
        <div className={`${styles.feedback} ${styles.feedbackIncorrect}`}>
          <strong>Not quite.</strong> The correct answer is: "{exercise.correctAnswer}"
        </div>
      )}
    </div>
  );
};

export default RemedialExercise;